...
说明：appId需使用土耳其里拉对应的，用户支付成功后增加土耳其里拉余额
...
| amount        | 是   | String | 交易金额（单位：本币，精确到小数点后两位；禁止添加标点符号，例如：”,”）                                                          |
| currency      | 否   | String | 币种(TRY)                                                                                                                        |
| payType       | 是   | String | 交易类型 PAYMENT\_GATEWAY                                                                                                        |
...
| fee          | String  | 手续费                                                      |
| status       | Integer | 状态(0未开始,1收款中,2收款成功3:收款失败, \-1异常待确认)    |
...
| appId         | 是   | Long   | 业务ID（后台获取，需要根据URL中的币种传递对应的业务ID）                                                                                                  |
| orderId       | 是   | String | 商户订单号(必须保证唯一性，长度不超过48)                                                                                                                 |
| amount        | 是   | String | 交易金额（单位：本币）                                                                                                                                   |
| accountType   | 是   | String | 账户类型： BANK_ACCOUNT,EWALLET                                                                                                                          |
| bankCode      | 是   | String | [银行编码](#银行编码)                                                                                                                                    |
...
| accountNo     | 是   | String | 用户账号(BANK_ACCOUNT为iban)                                                                                                                             |
| name          | 是   | String | 收款人姓名                                                                                                                                               |
...
| TRY  | BANK_ACCOUNT          | YAPI_KREDI         | 10-99000   | 维护 | YAPI ve KREDİ BANKASI A.Ş..         |
...
| TRY  | BANK_ACCOUNT          | ING_BANK           | 10-99000   | 维护 | ING BANK A.Ş..                      |
