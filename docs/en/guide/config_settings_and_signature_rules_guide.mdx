...
You can generate key pairs using the HaiPay Developer Tools, which are implemented purely in JavaScript and will not interact with the HaiPay server, ensuring that merchant key information is not leaked.  <a href="/en/rsa/rsa_generate/" target="_blank">Generates addresses</a>, alternatively, other online tools can be used to generate the key pair.
...
::: details Generate Signature String
...
::: details Signature Verification Test Demo
::: tabs
@tab java
```java
