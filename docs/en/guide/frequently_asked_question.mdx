...
## General
<Accordion title="After logging into HaiPay, there are no other functions besides the homepage.">
You need to authorize the previous sub-account roles again with the main account.
</Accordion>

{/* Old system */}
<Accordion title="Error when triggering payment, error message: No permission to pay for products, please contact the administrator.">
Possible reasons:
1. The client's product configuration is incorrect.
2. The payment method requested by the client is incorrect.
</Accordion>

<Accordion title="Error when triggering payment, response body: ">
```json
...
<Accordion title="Customer payment error, authentication failure message.">
Possible reason: The bank's risk control mechanism was triggered, so it was rejected. Try using a different card to retry.
</Accordion>

<Accordion title="What is the phone number format? <a id=\"phone_format\"></a>">
...
<Accordion title="Some payment methods fail to trigger the app, how to handle this?">
![](/images/app.png)
```java
...
## Taiwan
<Accordion title="Are there any issues with convenience store or ATM payments? Low success rate.">
...
## Brazil
<Accordion title="What should be filled in the \"Cash Source\" column in CDD?">
Game operation.
</Accordion>

<Accordion title="PIX collection order failed, error message: FAILED-invalid cpf or cnpj.">
This could be due to missing CPF.
</Accordion>

## PayPal / ACH / ECASHAPP
<Accordion title="When testing the disbursement API, is the name and account number required to be real accounts?">
The name can be random, but the recipient's account number must be real.
</Accordion>

<Accordion title="PayPal disbursement error, reason: bank return.">
The user's account has been flagged by PayPal, and payments cannot be received.
</Accordion>

<Accordion title="Issue with PayPal disbursement mistake recovery.">
...
<Accordion title="How long does it take for PayPal withdrawals to be credited?">
...
<Accordion title="Is the email field required when the bankCode is PayPal?">
Yes, it is required because the account is the email address.
</Accordion>

## Merchant Payout & Funding

<Accordion title="What is the Recharge process?">
...
<Accordion title="What should I pay attention to when recharging?">
...
<Accordion title="How to bind or change the login email?">
...
<Accordion title="How to initiate a single payout? What are the precautions?">
...
<Accordion title="How to initiate batch payout? What are the precautions?">
...
<Accordion title="How to download the payout (remittance) voucher after a successful payout?">
...
<Accordion title="Where can I view the Balance details and fees?">
